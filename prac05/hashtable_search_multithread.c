...
void htab_search_value(htab_t *h, int search)
{
	...
	thdata threaddata[THREADS];
	for (int i = 0; i < THREADS; i++) {
		threaddata[i].h = h;
		threaddata[i].search = search;
		threaddata[i].begin = i * h->size / THREADS;
		threaddata[i].end = (i + 1) * h->size / THREADS;
		pthread_create(&threads[i], NULL, thread_search, &threaddata[i]);
	}	
	for (int i = 0; i < THREADS; i++) {
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < 1000000; i++) {
        ...
        key[2] = ((i / 676) % 26) + 'A';
        key[3] = '\0';
        // Generate a value (0-9999) for each item
        htab_add(&h, key, rand() % 10000);
    }
